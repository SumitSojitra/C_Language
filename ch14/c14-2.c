...
void main(){
	...
	p1 = fopen("E:\\FLUTTER\\ch14\\mark.txt","a");
	...
	fprintf(p1,"ID\tNAME\tSUB1\tSUB2\tSUB3\tSUB4\tSUB5\tTOTAL\tPER\tGRADE\n--------------------------------------------------------------\n");
	char grade[10];
	for(i=0 ; i<n; i++){
		total[i] = s[i].sub1+s[i].sub2+s[i].sub3+s[i].sub4+s[i].sub5;
		per[i] = total[i]/5;
	
		fprintf(p1,"%d\t%s\t%d\t%d\t%d\t%d\t%d\t%d\t%.2f\t",s[i].id,s[i].name,s[i].sub1,s[i].sub2,s[i].sub3,s[i].sub4,s[i].sub5,total[i],per[i]);
